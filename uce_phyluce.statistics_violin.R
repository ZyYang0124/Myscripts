...
# 添加一个虚拟分组变量（用于 x 轴）
df$x <- "Species"  # 或 "All", "UCE Count", 等
...
# 绘制小提琴图
p <- ggplot(df, aes(x = x, y = n)) +  # ✅ 关键：指定 x = x
  ...
  annotate("text", x = 1.2, y = mean_n + (max_n - min_n)*0.02,
           label = paste("Mean =", mean_n), color = "blue", size = 4) +
  labs(
    title = "Distribution of UCE Loci Count Across 341 Species",
  ...
  theme(
    ...
    axis.text.x = element_blank(),  # 隐藏 x 轴文本（因为只有一个组）
...
# 保存图表
png_output <- file.path(output_dir, "uce_count_distribution_violin.png")
pdf_output <- file.path(output_dir, "uce_count_distribution_violin.pdf")

ggsave(png_output, p, width = 8, height = 6, dpi = 300, bg = "white")
ggsave(pdf_output, p, width = 8, height = 6, bg = "white")
